...
tls_config() {
	...
	while :; do
		echo
		echo -e "请输入一个 $magenta正确的域名$none，可以使用二级域名，但一定一定一定要正确，不！能！出！错！"
		read -p "(例如：blog.yangchengcaizi.cn): " domain
...
path_config() {
	echo
	while :; do
		echo -e "请输入想要 ${magenta}用来分流的路径$none , 例如 /yangchengcaizi , 那么只需要输入 yangchengcaizi 即可"
		read -p "$(echo -e "(默认: [${cyan}yangchengcaizi$none]):")" path
		[[ -z $path ]] && path="yangchengcaizi"
...
shadowsocks_password_config() {
	...
	while :; do
		...
		read -p "$(echo -e "(默认密码: ${cyan}blog.yangchengcaizi.cn$none)"): " sspass
		[ -z "$sspass" ] && sspass="blog.yangchengcaizi.cn"
		case $sspass in
		*[/$]*)
			echo
...
install_v2ray() {
	...
	if [[ $local_install ]]; then
		if [[ ! -d $(pwd)/config ]]; then
			...
			echo
			echo -e " 请确保你有完整的上传 洋城才子 的 V2Ray 一键安装脚本 & 管理脚本到当前 ${green}$(pwd) $none目录下"
			echo
			exit 1
		fi
		mkdir -p /etc/v2ray/yangchengcaizi/v2ray
		cp -rf $(pwd)/* /etc/v2ray/yangchengcaizi/v2ray
	else
		pushd /tmp
		git clone https://github.com/233boy/v2ray -b "$_gitbranch" /etc/v2ray/yangchengcaizi/v2ray --depth=1
		popd

	fi

	if [[ ! -d /etc/v2ray/yangchengcaizi/v2ray ]]; then
		echo
...
config() {
	cp -f /etc/v2ray/yangchengcaizi/v2ray/config/backup.conf $backup
	cp -f /etc/v2ray/yangchengcaizi/v2ray/v2ray.sh $_v2ray_sh
	chmod +x $_v2ray_sh

	v2ray_id=$uuid
	alterId=yangchengcaizi
	ban_bt=true
	if [[ $v2ray_transport -ge 18 ]]; then
...
backup_config() {
	...
	if [[ $shadowsocks ]]; then
		sed -i "42s/=/=true/; 45s/=6666/=$ssport/; 48s/=yangchengcaizi.cn/=$sspass/; 51s/=chacha20-ietf/=$ssciphers/" $backup
	fi
	[[ $v2ray_transport == [45] ]] && sed -i "36s/=yangchengcaizi.cn/=$domain/" $backup
	[[ $caddy ]] && sed -i "39s/=/=true/" $backup
	[[ $ban_ad ]] && sed -i "54s/=/=true/" $backup
	if [[ $is_path ]]; then
		sed -i "57s/=/=true/; 60s/=yangchengcaizi/=$path/" $backup
...
install() {
	if [[ -f /usr/bin/v2ray/v2ray && -f /etc/v2ray/config.json ]] && [[ -f $backup && -d /etc/v2ray/yangchengcaizi/v2ray ]]; then
		echo
		...
		echo
		exit 1
	elif [[ -f /usr/bin/v2ray/v2ray && -f /etc/v2ray/config.json ]] && [[ -f /etc/v2ray/yangchengcaizi_v2ray_backup.txt && -d /etc/v2ray/yangchengcaizi/v2ray ]]; then
		echo
...
uninstall() {
	...
	if [[ -f /usr/bin/v2ray/v2ray && -f /etc/v2ray/config.json ]] && [[ -f $backup && -d /etc/v2ray/yangchengcaizi/v2ray ]]; then
		...
		备注...仅支持卸载使洋城才子或233blog提供的 V2Ray 一键安装脚本
		" && exit 1
	fi
...
while :; do
	echo
	echo "........... V2Ray 一键安装脚本 & 管理脚本 by blog.yangchengcaizi.cn 原作者(233blog.com) .........."
	echo
	echo "帮助说明: https://yangchengcaizi.cn"
	echo
	echo "搭建教程: https://yangchengcaizi.cn"
	echo
	echo " 1. 安装"
	echo
	echo " 2. 卸载"
